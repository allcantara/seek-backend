...
module.exports = {
  async store(req, res) {
    try {
      ...
      if (!(await Restaurant.findById(restaurantId)))
        return res.status(203).send({ message: "Este restaurante não existe!" })
  ...
  async update(req, res) {
    try {
      ...
      const data = {
        image: req.file ? req.file.filename : productExist.image,
        name,
        ingredients: ingredientsList,
        price,
        updatedAt: Date.now(),
      ...
      console.log(error)
      return res.status(500).send({ message: "Falha na requisição!", error })
    }
  },
