...
module.exports = {
  async validateFieldsStore(
    ...
    if (!device)
      return {
        error: true,
        message: "Não foi possível identificar o tipo de dispositivo!"
      };

    if (!restaurantId)
      return {
        error: true,
        message: "Não foi possível identificar o restaurante!"
      };

    if (!productsList || productsList.length === 0)
      return {
        error: true,
        message: "Não foi possível identificar os produtos!"
      };

    if (!payment)
      return {
        error: true,
        message: "A forma de pagamento deve ser informada!"
      };
    ...
    if (device === devicesConfig.APP) {
      if (!userId || userId === "")
        return {
          error: true,
          message: "Não foi possível identificar o usuário da compra!"
        };
    ...
    if (device === devicesConfig.WEB) {
      if (!tableNumber)
        return { error: true, message: "Por favor, informe o número da mesa!" };
      if (!clientName)
        return { error: true, message: "Por favor, informe seu nome!" };
    }
    ...
    productsList.map(async product => {
      ...
      if (!(await Product.findById(itemId)))
        return {
          error: true,
          message: "Algum dos produtos informado não possui cadastro!"
        };
    });
